...
def support_get_global_average_vote():
  ...
  global_avg = pd.read_sql(query, engine).iloc[0][0]
  return round(global_avg, 2)


def compute_our_rating(m_percentile: int, delete_existing: bool = None, use_given_m = None):
  assert type(m_percentile) == int and m_percentile > 0 and m_percentile <= 100, "Error, bad 'm_percentile' choosen, choose a value between 0 and 100 (both included)."
  ...
  if delete_existing == True: # cancello i ratings, per quel 'min_votes'
  ...
  for i, id_game in enumerate(my_dict['v_id_games_todo']):
    ...
    query = """INSERT INTO rating (id_game, min_votes, m_percentile, global_avg, count, mean, median, std, rating)
